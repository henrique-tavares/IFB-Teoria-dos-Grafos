...
if __name__ == "__main__":
    ...
    print(
        "Gasto de memória (kb)",
        f"Grafo lista: {graph_type_list_memusg}",
        f"Grafo matriz: {graph_type_matrix_memusg}",
        sep="\n",
    )
    ...
    with open(input_path) as text_file:
        ...
        print(f"Tempo grafo matriz: {time_matrix}s")

        start = time.time()
        g_list.out_graph(out_path)
        end = time.time()
        time_list = end - start
        print(f"Tempo grafo lista: {time_list}s")
